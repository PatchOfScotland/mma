-- Based on: https://github.com/diku-dk/futhark-benchmarks/blob/abac6074cce1cb55f800b16f8732dc96081c4add/rodinia/lud/lud.fut
-- Experiment notes:
-- Altering block size to change array multiplication
-- Alter problem size so constant amount of work

-- Parallel blocked LU-decomposition.
-- ==
-- "N:16 " compiled random input { 16i64 [256][256]f16 }
-- "N:32 " compiled random input { 32i64 [512][512]f16 }
-- "N:64 " compiled random input { 64i64 [1024][1024]f16 }
-- "N:128 " compiled random input { 128i64 [2048][2048]f16 }

def dotprod [n] (a: [n]f16) (b: [n]f16): f16 =
  map2 (*) a b
  |> reduce (+) 0

#[inline]
def lud_diagonal [b] (a: [b][b]f16): *[b][b]f16 =
  map (\mat ->
         let mat = copy mat
         in #[unsafe]
            loop (mat: *[b][b]f16) for i < b-1 do
            ...
            in mat
      ) (take (opaque 1) (unflatten (a :> [1*b][b]f16)))
  |> head

def lud_perimeter_upper [m][b] (diag: [b][b]f16) (a0s: [m][b][b]f16): *[m][b][b]f16 =
    let a1s = map (\ (x: [b][b]f16): [b][b]f16  -> transpose(x)) a0s in
    let a2s =
        map (\a1 ->
               map (\row0 -> -- Upper
                      #[unsafe]
                      loop row = copy row0 for i < b do
                      let sum = loop sum=0.0f16 for k < i do
                                  sum + diag[i,k] * row[k]
                      let row[i] = row[i] - sum
                      in  row
                   ) a1
...
def lud_perimeter_lower [b][m] (diag: [b][b]f16) (mat: [m][b][b]f16): *[m][b][b]f16 =
  map (\blk ->
         map (\row0 -> -- Lower
                #[unsafe]
                loop row = copy row0 for j < b do
                let sum = loop sum=0.0f16 for k < j do
                            sum + diag[k,j] * row[k]
...
def lud_internal [m][b] (top_per: [m][b][b]f16) (lft_per: [m][b][b]f16) (mat_slice: [m][m][b][b]f16): *[m][m][b][b]f16 =
...
def main [m] (bb: i64) (mat: [m][m]f16): [m][m]f16 =
    let b = bb
