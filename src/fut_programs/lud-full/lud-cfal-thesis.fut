...
def matmulThesis [m][n][k] (A: [m][k]real) (B: [k][n]real) : [m][n]real =
  map (\Arow ->
    map (\Bcol ->
      map2 (*) Arow Bcol
        |> reduce (+) 0.0
...
def oneIter [d][m] (K: [m*d][d]real) (V: [m*d][d]real) (Qi: [d][d]real) : [d][d]real =
  let P_block = matmulThesis Qi (transpose K) |> opaque -- : [d][m*d]real 
  -- let P_block = softmax P_block
  let P_block = softmaxOnline P_block-- : [d][m*d]real
  in  matmulThesis P_block V      -- : [d][d]real
...
entry mk_input_d (m:i64) (d:i64) : ([m][d][d]real, [d][d]real, [d][d]real) =
  let Q = replicate d 1.0 |> replicate d |> replicate m
